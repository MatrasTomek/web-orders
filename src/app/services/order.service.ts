...
export class OrderService {
	...
	public async getOrders() {
		return new Promise<any>((resolve) => {
			this.orderCollection.valueChanges({ idField: 'id' }).subscribe((orders) => resolve(orders));
		});
	}

	createOrder(orderData: IOrder): Promise<DocumentReference<IOrder>> {
	...
	public async deleteOrder(orderId: string) {
		this.orderCollection.doc(orderId).delete();
	}
}
